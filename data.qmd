...
Here’s a breakdown of the unchanged variables from the original data set:


```{r echo = FALSE, results = 'show'}
table <- data.frame(Variable = c( "incident_num", "shooting_date", "district", "shooting_type_v2", "victim_gender", "victim_race", "victim_ethnicity_NIBRS", "multi_victim"),
                  Type = c("integer", "date", "string", "string", "string", "string", "string", "boolean"),
                  Description = c("Unique identifier for each shooting incident.", "Date when the shooting occurred.", "Police district where the incident was reported", "Specifies if the shooting was fatal or non-fatal.", "Gender of the victim involved.", "Race of the victim.", "Ethnicity of the victim according to NIBRS standards.", "Indicates if multiple victims were involved."))
...
We are considering key indicators such as employment status, unemployment rate, median income, percent of single-parent households, and percentage of homes that are owner occupied.
...
To obtain socioeconomic and demographic data, we utilized the tidycensus package to query census data. For our analysis, we utilized MASS to preform stepwise selection, GWmodel for geospatial regression, car to calculate VIF, sf to work with spatial data, segregation to calculate segregation index, ggpmisc to include statistical summaries in our plots, stargazer to formalize our regression results,corrplot for our correlation matrix, and tigris to color our maps. Additionally, we relied on chapter 7 and 8 from Analyzing US Census data for our analysis. 
